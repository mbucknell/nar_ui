...
if(plot.series[neighbor.seriesIndex].show&&hl.showTooltip&&(!c||!c._zoom.started)){showTooltip(plot,plot.series[neighbor.seriesIndex],neighbor);}
if(hl.bringSeriesToFront){plot.moveSeriesToFront(neighbor.seriesIndex);}}}}}})(jQuery);var nar=nar||{};(function(){nar.Constituents={ammonia:{color:'#CAFF70',name:'Ammonia'},orthoP:{color:'#C1FFC1',name:'Ortho Phosphate'},nitrogen:{color:'#6E8B3D',name:'Total Nitrogen'},pesticides:{color:'#008080',name:'Pesticides'},ecology:{color:'#808000',name:'Ecology'},nitrate:{color:'#7FFF00',name:'Nitrate'},phosphorus:{color:'#698B69',name:'Total Phosphorus'},sediment:{color:'#8B0000',name:'Suspended Sediment'},streamflow:{color:'#0000CD',name:'Streamflow'}};})();var nar=nar||{};nar.loadSiteHelpInfo=function(url){nar.siteHelpInfo={};nar.siteHelpInfoPromise=$.ajax({url:url,type:'GET',data:{site_id:PARAMS.siteId},success:function(data){nar.siteHelpInfo=data;},error:function(jqXHR,textStatus,errorThrown){throw Error('Unable to contact the site info URL service: '&textStatus);}});};var nar=nar||{};nar.informativePopup=function(args){"use strict";var me=(this===window)?{}:this,bodyText='body',mouseEnterText='mouseenter',clickText='click',selector=args.selector||$(bodyText),$anchor=args.$anchor,popover,show=args.show||function(){var popoverObject=$anchor.data('bs.popover');popover.off(mouseEnterText,show);$anchor.on('shown.bs.popover',function(){popoverObject.$tip.find('.popover-close').on(clickText,hide);});$anchor.popover('show');},hide=args.hide||function(){$anchor.popover('hide');popover.on(mouseEnterText,show);};args.title=args.title||'';args.title+='&nbsp;<span class="popover-close glyphicon glyphicon-remove pull-right"></span>';popover=$anchor.popover($.extend({},{html:true,content:'Placeholder content',container:bodyText,selector:bodyText,trigger:'manual',placement:'auto',template:'<div class="popover summary-popover" role="tooltip"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>'},args));popover.on(mouseEnterText,show);popover.on(clickText,hide);$(bodyText).on('resize',hide);};var nar=nar||{};nar.plots=nar.plots||{};(function(){var ConstituentCurrentYearComparisonPlot=function(plotContainerSelector,series,legendSelector){nar.util.assert_selector_present(plotContainerSelector);if(legendSelector){nar.util.assert_selector_present(legendSelector);}
var averageColor=series.averageColor||ConstituentCurrentYearComparisonPlot.defaultAverageColor;var plotContainer=$(plotContainerSelector);var plotClass=ConstituentCurrentYearComparisonPlot.plotClass;var plotDiv=$('<div/>',{'class':plotClass});var plotDivSelector=plotContainerSelector+' .'+plotClass;plotContainer.append(plotDiv);var legendElt=$('<div/>',{'class':ConstituentCurrentYearComparisonPlot.legendClass});var yearLegendClass=ConstituentCurrentYearComparisonPlot.yearLegendClass;var yearLegendElt=$('<span/>',{'class':yearLegendClass});yearLegendElt.html('2014');var yearLegendEltSelector=plotContainerSelector+' .'+yearLegendClass;var averageLegendClass=ConstituentCurrentYearComparisonPlot.averageLegendClass;var averageLegendElt=$('<span/>',{'class':averageLegendClass});averageLegendElt.html('Avg');legendElt.append(averageLegendElt);legendElt.append(yearLegendElt);plotContainer.append(legendElt);var titleClass=ConstituentCurrentYearComparisonPlot.titleClass;var titleDiv=$('<div/>',{'class':titleClass});titleDiv.html(series.constituentName);var titleDivSelector=plotContainerSelector+' .'+titleClass;plotContainer.append(titleDiv);var yearSeries={data:[['',series.yearValue]],label:'2014',bars:{show:true,barWidth:4,align:"left",fillColor:series.yearColor}};var averageSeries={data:[['',series.averageValue]],label:series.averageName,bars:{show:true,barWidth:4,align:"right",fillColor:averageColor}};var flotSeries=[averageSeries,yearSeries];var plot=$.plot(plotDivSelector,flotSeries,{xaxis:{mode:"categories",tickLength:0},yaxis:{axisLabel:series.displayUnitOnYAxis?series.constituentUnit:null,axisLabelUseCanvas:true,axisLabelPadding:3,tickLength:3,tickColor:'#000000'},grid:{hoverable:true,clickable:true},colors:[averageColor,series.yearColor],tooltip:true,tooltipOpts:{content:'%s: %y '+series.constituentUnit},legend:{show:false}});if(series.yearValue===0){var NO_DATA='No data';var ctx=plot.getCanvas().getContext("2d");var xaxis=plot.getAxes().xaxis;var yaxis=plot.getAxes().yaxis;var plotOffset=plot.getPlotOffset();var metrics=ctx.measureText(NO_DATA);var tx=xaxis.p2c(2)+plotOffset.left;var ty=yaxis.p2c(yaxis.max/2)+plotOffset.bottom;ctx.save();ctx.translate(tx,ty);ctx.rotate(-0.5*Math.PI);ctx.fillText("No data",-metrics.width/2,4);ctx.restore();}
return plot;};ConstituentCurrentYearComparisonPlot.defaultAverageColor='#D8DCDC';ConstituentCurrentYearComparisonPlot.titleClass='currentYearComparisonTitle';ConstituentCurrentYearComparisonPlot.yearLegendClass='currentYearComparisonYearLegend';ConstituentCurrentYearComparisonPlot.averageLegendClass='currentYearComparisonAverageLegend';ConstituentCurrentYearComparisonPlot.legendClass='comparisonLegend';ConstituentCurrentYearComparisonPlot.plotClass='currentYearComparisonPlot';nar.plots.ConstituentCurrentYearComparisonPlot=ConstituentCurrentYearComparisonPlot;}());var nar=nar||{};nar.plots=nar.plots||{};(function(){var ExceedancePlot=function(plotEltId,seriesSpecifications,axisLabel){nar.util.assert_selector_present('#'+plotEltId);$.jqplot.config.enablePlugins=true;var data=seriesSpecifications.map(function(seriesSpec){return[seriesSpec.data,seriesSpec.constituent.name];});var seriesOptions=seriesSpecifications.map(function(seriesSpec){return seriesSpec.constituent.color;});var labels=seriesSpecifications.map(function(seriesSpec){return seriesSpec.label;});var plot=$.jqplot(plotEltId,[data],{seriesDefaults:{renderer:$.jqplot.BarRenderer,rendererOptions:{barDirection:'horizontal',varyBarColor:true,barWidth:40,shadowDepth:0},pointLabels:{show:true,location:'e',edgeTolerance:-30,labels:labels}},seriesColors:seriesOptions,axes:{yaxis:{renderer:$.jqplot.CategoryAxisRenderer,tickRenderer:$.jqplot.AxisTickRenderer,tickOptions:{labelPosition:'middle',showGridline:false}},xaxis:{max:100,min:0,label:axisLabel}},grid:{drawGridLine:false},highlighter:{show:true,tooltipLocation:'n',tooltipOffset:15,formatString:'EPA MCL = 10 mg/L as N. See Technical Information for details.'}});$(window).resize(function(){plot.replot();});return plot;};nar.plots.ExceedancePlot=ExceedancePlot;}());$(document).ready(function(){$.when(nar.siteHelpInfoPromise).done(function(){var isSiteForDummyNullData=PARAMS.siteId==='05451210';var ConstituentCurrentYearComparisonPlot=nar.plots.ConstituentCurrentYearComparisonPlot;var ExceedancePlot=nar.plots.ExceedancePlot;var exceedancePlotLabel=function(data){var result=[];data.forEach(function(value){if(value===0){result.push('No detections above benchmarks');}
else{result.push(value);}});return result;};var nitrateSeries={constituentName:nar.Constituents.nitrate.name,constituentUnit:'Million Tons',yearValue:isSiteForDummyNullData?0:12,yearColor:nar.Constituents.nitrate.color,averageName:'Average 1991-2013',averageValue:10};var nitrateGraph=ConstituentCurrentYearComparisonPlot('#barChart2',nitrateSeries);var phosphorusSeries={constituentName:nar.Constituents.phosphorus.name,constituentUnit:'Million Tons',yearValue:isSiteForDummyNullData?0:100,yearColor:nar.Constituents.phosphorus.color,averageName:'Average 1985-2013',averageValue:153};var phosphorusGraph=ConstituentCurrentYearComparisonPlot('#barChart3',phosphorusSeries);var streamflowSeries={constituentName:nar.Constituents.streamflow.name,constituentUnit:'Million acre-feet',yearValue:isSiteForDummyNullData?0:1.7,yearColor:nar.Constituents.streamflow.color,averageName:'Average 1999-2013',averageValue:2.4};var streamflowGraph=ConstituentCurrentYearComparisonPlot('#barChart1',streamflowSeries);var sedimentSeries={constituentName:nar.Constituents.sediment.name,constituentUnit:'Million Tons',yearValue:isSiteForDummyNullData?0:300,yearColor:nar.Constituents.sediment.color,averageName:'Average 1990-2013',averageValue:100};var sedimentGraph=ConstituentCurrentYearComparisonPlot('#barChart4',sedimentSeries);var exceedancesTitle='Percent of samples with concentrations greater than benchmarks';var nitrateData=isSiteForDummyNullData?[0]:[73];var humanHealthExceedancePlot=ExceedancePlot('humanHealthExceedances',[{constituent:nar.Constituents.nitrate,data:nitrateData,label:exceedancePlotLabel(nitrateData)},{constituent:{color:'',name:' '},data:[' '],label:['']}],exceedancesTitle);nar.informativePopup({$anchor:$('#link-hover-benchmark-human'),content:'<div class="popover-benchmark-content">\
